...
static void test_parse_put_object_path_style(void) {
    ...
    int result = s3_route_parse("PUT", "/mybucket/mykey.txt",
                             headers, 4, &request);
    assert(result== 0);
    assert(strcmp(request.bucket, "mybucket") == 0);
    assert(strcmp(request.key, "mykey.txt") == 0);
    assert(request.operation ==S3_OPERATION_PUT_OBJECT);
...
static void test_parse_get_object(void) {
    ...
    int result = s3_route_parse("GET", "/mybucket/data/file.csv",
                             NULL, 0, &request);
    assert(result== 0);
    assert(strcmp(request.bucket, "mybucket") == 0);
    assert(strcmp(request.key, "data/file.csv") == 0);
    assert(request.operation ==S3_OPERATION_GET_OBJECT);
...
static void test_parse_delete_object(void) {
    ...
    int result = s3_route_parse("DELETE", "/mybucket/old-file.txt",
                             NULL, 0, &request);
    assert(result== 0);
    assert(strcmp(request.bucket, "mybucket") == 0);
    assert(strcmp(request.key, "old-file.txt") == 0);
    assert(request.operation ==S3_OPERATION_DELETE_OBJECT);
...
static void test_parse_list_objects(void) {
    ...
    int result = s3_route_parse("GET", "/mybucket?prefix=logs/&max-keys=100",
                             NULL, 0, &request);
    assert(result== 0);
    assert(strcmp(request.bucket, "mybucket") == 0);
    assert(request.key[0] == '\0');
    assert(request.operation ==S3_OPERATION_LIST_OBJECTS);
...
static void test_parse_unknown_method(void) {
    ...
    int result = s3_route_parse("PATCH", "/mybucket/key",
                             NULL, 0, &request);
    assert(result!= 0);
